...
func _physics_process(delta: float) -> void:
	...
	if holding_input:
		hold_time = clampf(hold_time + delta, 0 , hold_duration)
	else:
		hold_time = 0
	if _is_grabbed:
...
func _while_grabbed(controller: InteractionController) -> void:
	holding_input = true
	if hold_time < hold_duration: return
...
func loot_object():
	...
	tween.set_parallel()
	tween.tween_property(self, "global_position", Global.player.enemy_look_at.global_position, .25).set_trans(Tween.TRANS_CIRC)
