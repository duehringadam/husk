...
func _ready() -> void:
	...
	SignalBus.connect("player_stats_changed", func(_stats: Dictionary):set_max_stamina(base_stamina))
	SignalBus.connect("player_full_restore", func():modify_stamina(max_stamina))
