...
func update_total_loading_progress() -> void:
	var partial_total: float = _scene_loading_progress
...
func _show_all_draw_passes_once() -> void:
	var all_materials: PackedStringArray = _traverse_folders(_spatial_shader_material_dir)
	var total_material_count: int = all_materials.size()
	var cached_material_count: int = 0
	for material_path: String in all_materials:
...
func _traverse_folders(dir_path:String) -> PackedStringArray:
	...
	while file_name != "":
		if not dir.current_is_dir():
			...
			for extension: String in _matching_extensions:
			...
			var subfolder_name: String = file_name
...
func _load_material(path:String) -> void:
	var material_shower: MeshInstance3D = MeshInstance3D.new()
	material_shower.mesh = _mesh
	var material: Material = ResourceLoader.load(path)
