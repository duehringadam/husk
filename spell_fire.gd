...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed("attack_secondary"):
		...
		ray.look_at(end)
		AudioManager.play_sound(load("res://sfx/fire-magic-5-378639.mp3"), self.global_position, -15)
		if ray.is_colliding():
			...
			AudioManager.play_sound(load("res://sfx/whoosh-flame-388763.wav"),ray.get_collision_point(),-15)
			if global_position.distance_to(Global.player.global_position) < 5:
				Global.player.camera.apply_shake(0.06)
		else:
			var spell_projectile_add = spell_projectile.instantiate()
			get_tree().current_scene.add_child(spell_projectile_add)
			AudioManager.play_sound(load("res://sfx/whoosh-flame-388763.wav"),end,-15)
